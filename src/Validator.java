...
public class Validator {
    ...
    private static void check (String login, String password, String confirmPassword)
        ...
        if (Objects.isNull(login) || login.length() > 20) {
            throw new WrongLoginException ("Длина логина должна быть меньше или равна 20 символов!");
        }
        ...
        if (!checker.check(login)) {
            throw new WrongLoginException("Логин содержит неккоректные символы");
        }
        if (!checker.check(password)) {
            throw new WrongPasswordException("Пароль содержит неккоректные символы");
        }
    }
}
